...
SerialTerminal::SerialTerminal(QWidget *parent) :
    QTextEdit(parent), m_escape{nullptr}
{
...
void SerialTerminal::setEscape(TerminalEscape* escape)
{
    m_escape = escape;
}
...
void SerialTerminal::putData(const QByteArray &_data)
{
    ...
    if (m_escape != nullptr)
    {
        TerminalData data(_data);
        UnescapedTerminalData udata = m_escape->unescape(data);
